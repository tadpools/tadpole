...
Levels: `Light` (rate limits, heartbeat hiccups — one friendly line),
`Actionable` (REST/decode errors — notes plus a straight `Try:` list),
`Severe` (auth failures — zero whimsy), `Internal` (our bugs — apology and
a report request). Pond words (the Current, lilypads, ripples) may flavor
